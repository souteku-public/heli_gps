...
市区町村コード表（muni.json）:
  国土地理院（地理院地図 muni.js）を加工して作成 ※表記義務なし（問合せ番号 91803）
郡名テーブル（gun.json）:
  geolonia/japanese-addresses (CC0-1.0) より作成 ※表記義務なし
```

放送・公衆送信時に**表記義務があるのは国土交通省分のみ**です。
国土地理院へは直接照会し、出典明示・契約ともに不要との回答を得ています
（問合せ番号 91803、`../docs/terms/gsi_inquiry_91803.md`）。
