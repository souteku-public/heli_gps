...
├── docs\terms\              利用規約の取得時点スナップショット
├── THIRD_PARTY_NOTICES.md   出典・ライセンス一覧
├── TERMS_OF_USE.md          利用条件（系列局向け）
...
**配布物には Noto Sans JP（SIL OFL・可変フォント）が同梱済み**です
（`fonts\NotoSansJP-VF.ttf` ＋ ライセンス全文 `fonts\OFL.txt`）。
これが**最優先**で使われるため、**全局で同一の見た目**になります。
文字の太さは制御UIの「見た目 → 文字の太さ」で変更できます（既定 Bold）。

> `fonts\OFL.txt` は OFL の必須条件です。**削除しないでください。**
...
制御UIの「**地図スーパー**」セクションで ON にすると、放送する映像に
ヘリ位置の小さな地図が重なります（インターネット不要・同梱データのみ）。

- **ON/OFF**: チェックボックス（**既定はOFF**。ONにした時だけ放送に出ます）
- **大きさ**: 「地図の大きさ(幅px)」スライダー／数値入力（高さは4:3で連動）
- **表示範囲**: 数字が小さいほど拡大（0.05〜1.2度）
- **位置**: 「配置」の枠内に出る水色の「地図」枠を**ドラッグ**
- **軌跡**: チェックで飛行経路を描画
- 変更は他の見た目設定と同じく「**決定（反映）**」で反映され、
  番組プリセットにも保存されます

> **出典クレジットは地図内に自動で焼き込まれます（消せません）。**
> これは利用規約上の要件です。隠す・切り取る等の加工をしないでください。
...
## 10. ★重要：表示される地名の限界（必ず運用者へ周知）

住所判定に使う境界データには、その利用規約（第2条）により次の限界があり、
**最終利用者まで伝達すべき**とされています。

1. 位置の代表点は**街区・大字レベル（概ね数100メートル）の公差**を持ちます
2. データは**1〜5年程度のタイムラグ**を有します（市町村合併・境界変更の反映遅れ）
3. したがって **市区町村の境界付近では表示される地名が実際と異なる場合があります**

**運用のお願い**: 放送に出す前に、表示される地名を**オペレーターが目視確認**して
ください。境界付近・離島・海上では特にご注意ください
（海上は最寄り市町村に「沖」を付けて表示します）。
...
- 本システムは**外部の地図サービス・APIキー・アカウント連携を使用しません**
- 放送に出る情報の出所は次の2つです
  - 地図の境界形状 …「**国土数値情報（行政区域データ）**」（国土交通省）を加工して作成
  - 地名の文字列 … **国土地理院**（地理院地図）のデータを加工して作成
- **国土地理院への出典表記・契約はいずれも不要**です。
  同院へ直接照会し、公式回答を得ています（問合せ番号 91803。
  `docs\terms\gsi_inquiry_91803.md` に照会文・回答文を保存）
- **国土数値情報（国土交通省）の出典表記**は、地図スーパーON時は画面内に自動描画されます。
  地図OFF時は番組クレジット等で掲出してください。掲出例:
  > この番組の位置情報は、「国土数値情報（行政区域データ）」（国土交通省）
  > のデータを加工して作成しています。
- **測量法第29条・第30条の承認申請は不要**です（地理院タイル等の測量成果を使用しておらず、
  国土地理院からも契約不要との回答を得ています）
- 同梱フォント（Noto Sans JP / SIL OFL 1.1）は**放送での使用に制限がありません**
- 詳細は次の文書を参照のうえ、自局の法務にご確認ください:
  - `TERMS_OF_USE.md` … 利用条件（再配布禁止・出典義務・適用限界の継承）
  - `docs\ATTRIBUTION.md` … **出典表記の対象・文言・掲出先（正式パッケージ名つき）**
  - `docs\LICENSING.md` … ライセンス整理
