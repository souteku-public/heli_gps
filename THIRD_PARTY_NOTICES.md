...
放送・公衆送信・配布物に用いる場合、次の**出典記載**と**加工した旨の記載**が必要です。
...
**国土地理院分（`muni.json`）の出典表記は不要**です。同院へ直接照会し、
「当院から当院に関する出典明示等は求めません／当院と契約書を交わす必要もございません」
との公式回答を得ています（問合せ番号 91803、`docs/terms/gsi_inquiry_91803.md`）。

地図スーパーON時は、地図インセット内に
`出典:国土数値情報(国土交通省)・国土地理院を加工して作成` が**自動描画されます（消去不可）**。
国土地理院分を併記しているのは、上記回答が「住所を文字スーパーする使用形態」に
限定されており**地図描画は射程外**のため、安全側に倒しているものです。

**地理院タイル等の測量成果は使用しておらず、測量法第29条・第30条の承認申請は不要です。**
