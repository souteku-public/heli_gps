...
| **文字サイズ** | スライダー |
| 横位置基準 / 縦位置基準 | 左中右 / 下中上 |
| **位置(X,Y)px** | セーフエリア内へ微調整。**上下左右キーでも動かせます**(下記) |
| 矢印キーの移動量(px) | 上下左右キー1回の移動量。Shift併用で10倍 |
| 文字色 / 不透明度 | カラーピッカー |

変更は overlay_text と gps_decode に**即座に反映**されます(内部で
パラメータバインド)。本番中もこのページだけ操作すれば完結します。

**町丁目(区の次)まで出したいとき**: 「住所変換モード」を **自動** に、
「住所の粒度」を **町丁目** にします。町丁目は国土地理院APIから取得する
ため**インターネット接続が必要**です(自動モードなら、ネットがある時だけ
町丁目・無い時は市区町村に自動フォールバック。実測で「千葉市美浜区浜田
二丁目」まで取得を確認済み)。ネット接続が難しい現場では、オフラインの
まま市区町村表示が確実です。

**上下左右キーでの位置調整**: ネットワーク内に `nudge_keys`(Keyboard In
DAT)を配置済みです。**↑↓←→キーで文字位置が1pxずつ動きます**
(Shift+矢印で10px)。移動量はControlsの「矢印キーの移動量」で変更可。
キーが効かない場合はTDのウィンドウがアクティブ(前面)になっているか
確認してください。
...
> ```
> (矢印キー機能も含めて追加するには build_network.py を再実行してください)

### 3-3b. 外部Python制御UI (操作しやすいパネル・状態表示つき)

TDのパラメータ画面より操作しやすい**別ウィンドウのPython製UI**から、
OSC経由でスーパーを操作できます。`build_network.py` を実行すると、
受け口の `osc_ctrl`(OSC In DAT, ポート9001)が自動配置されます。

起動(リポジトリ直下で):
```
python control_ui.py
```
(別PCのTDを操作する場合: `python control_ui.py --td-host <TDのIP>`)

- スライダー/ドロップダウン/カラーピッカーで、モード・粒度・フォント・
  文字サイズ・位置・色・書式をリアルタイム操作
- **↑↓←→キー**でも位置微調整(Shiftで×10)
- 下部に**受信状態**(受信中/途絶)・現在のスーパー文字・緯度経度・測位状態を
  ライブ表示(TDから9002で受信)
- 一括起動は `start_heli.bat`(TDと制御UIを同時に開く。冒頭のパスを環境に合わせる)

> 別PCで制御UIを動かす場合、TD→UIの状態表示を受けるには
> `gps_decode_callbacks.py` の `STATUS_OSC` を制御PCのIPに変更してください
> (操作だけなら不要)。
